...
class TestLRScheduler:
    ...
    def test_linear_warmup(self):
        ...
        s = LinearLR(lr=1e-3, warmup_steps=10, total_steps=0,
                     warmup_start_lr=0.0, end_lr=0.0)
    ...
    def test_linear_decay(self):
        from hermes_agentic_rl.trainers.lr_schedule import LinearLR
        s = LinearLR(lr=1e-3, warmup_steps=0, total_steps=100,
                     warmup_start_lr=0.0, end_lr=0.0)
        assert s.get_lr(0) == pytest.approx(1e-3)
    ...
    def test_cosine(self):
        ...
        s = CosineLR(lr=1e-3, total_steps=100, warmup_steps=0,
                     warmup_start_lr=0.0, end_lr=0.0)
        assert s.get_lr(0) == pytest.approx(1e-3)
    ...
    def test_warmup_cosine(self):
        ...
        s = make_lr_scheduler("warmup_cosine", lr=1e-3, warmup_steps=10,
                               total_steps=110, end_lr=1e-5)
        # During warmup
        assert s.get_lr(0) == pytest.approx(0.0)
        assert s.get_lr(10) == pytest.approx(1e-3)
    ...
    def test_grpo_dapo_mode(self):
        ...
        records = [
            RolloutRecord(
                ...
                old_logprobs=[-0.5, -0.5], reward=1.0, group_id="g0",
            ),
            RolloutRecord(
                prompt_ids=[1, 2], response_ids=[3, 4],
                old_logprobs=[-0.5, -0.5], reward=1.0, group_id="g0",
...
class TestExpandPerToken:
    ...
    def test_empty_response_passthrough(self):
        ...
        from hermes_agentic_rl.algos.common.reinforce_pp import expand_per_token_advantage
        rec = RolloutRecord(
            prompt_ids=[1], response_ids=[],
            old_logprobs=[], reward=1.0, group_id="g",
        )
    ...
    def test_rloo_per_token_advantage(self):
        ...
        records = [
            RolloutRecord(prompt_ids=[1, 2], response_ids=[3, 4, 5],
                          old_logprobs=[-0.3]*3, reward=0.8, group_id="g0"),
            RolloutRecord(prompt_ids=[1, 2], response_ids=[3, 4, 5],
                          old_logprobs=[-0.3]*3, reward=0.2, group_id="g0"),
        ]
...
class TestCheckpointNormState:
    ...
    def test_checkpoint_state_carries_running_stats(self):
        ...
        ks = {"value": 0.03, "target_kl": 0.1, "horizon": 10000.0,
              "min_coef": 1e-4, "max_coef": 10.0, "init_kl_coef": 0.05,
              "step_count": 2}
        state = CheckpointState(
            iteration=5,
            model_state={}, optimizer_state=None, rng_state=None,
            stats=[], config={}, best_reward=0.5, best_iteration=3,
            running_stats=rs, kl_ctrl_state=ks,
        )
...
class TestGradClipMonitor:
    def test_grad_clip_triggered_field_exists(self):
        ...
        cfg = GRPOTrainerConfig(n_iters=2, group_size=2, prompts_per_iter=1,
                                max_new_tokens=4, grad_clip=1e-6, log_every=0)
...
class TestPERBufferDeque:
    ...
    def test_sample_prioritized_after_eviction(self):
        ...
        for i in range(6):
            buf.add_sample(TrainSample(prompt_ids=[i], response_ids=[i],
                                       reward=float(i)), priority=float(i + 1))
...
class TestTrainStatsV10:
    def _make_stats(self):
        ...
        s = TrainStats()
        for i in range(5):
            s.add({"iter": i, "mean_reward": float(i) * 0.1,
...
class TestFormatLogDynamic:
    def _make_trainer(self):
        ...
        cfg = GRPOTrainerConfig(n_iters=1, group_size=2, prompts_per_iter=1,
                                max_new_tokens=3, log_every=0)
        return GRPOTrainer(policy=policy, env=env, reward_manager=rm, cfg=cfg)

    def test_primary_keys_present(self):
        trainer = self._make_trainer()
        rec = {"iter": 0, "algo": "grpo", "mean_reward": 0.5, "loss": 0.1,
               "kl": 0.01, "clip_frac": 0.2, "n_updated": 4}
    ...
    def test_extra_numeric_keys_appear(self):
        trainer = self._make_trainer()
        rec = {"iter": 0, "algo": "grpo", "mean_reward": 0.5, "loss": 0.1,
               "grad_norm": 0.123, "my_custom_metric": 42.0}
        log = trainer._format_log(rec)
    ...
    def test_suppressed_keys_absent(self):
        trainer = self._make_trainer()
        rec = {"iter": 0, "mean_reward": 0.5, "n_tokens": 999,
               "ratio_mean": 1.01, "optimizer_step_applied": 1.0}
        log = trainer._format_log(rec)
...
class TestRLOOAlgo:
    def test_rloo_basic(self):
        ...
        records = [
            RolloutRecord(prompt_ids=[1, 2], response_ids=[3, 4, 5],
                          old_logprobs=[-0.3]*3, reward=r, group_id="g0")
