...
BuilderFn = Callable[[dict], tuple[Any, Any, Any, Any]]
...
class MPRolloutPoolConfig:
    ...
    build_ctx: dict[str, Any] = field(default_factory=dict)
...
def _worker_main(
    ...
    def _sync_weights() -> None:
        ...
        if hasattr(backend, "model") and blob is not None:
            buf = io.BytesIO(blob)
            state = torch.load(buf, map_location="cpu")  # noqa: torch-load-unsafe
        ...
        if kind == "task":
            ...
            try:
                ...
                async def _run() -> dict[str, Any]:
                    traj = await RolloutManager(loop).collect(task.item, task.instruction)
                    summary = await reward_manager.evaluate(task.item, traj, tool_context=None)
                            ...
                            records.append(
                                {
                                    ...
                                    "group_id": str(task.item.get("task_id", "group")),
                                    "metadata": {**base_meta, "turn_index": ti},
                                }
                            )
                    else:
                        records.append(
...
class MPRolloutPool:
    ...
    def __init__(self, cfg: MPRolloutPoolConfig, builder_fn: BuilderFn) -> None:
        ...
        self._ctx = mp.get_context(cfg.ctx_method)
